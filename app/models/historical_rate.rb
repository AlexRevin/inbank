...
class HistoricalRate < CurrencyBase
  ...
  def self.for(code)
    joins('left join currencies source_currencies on historical_rates.source_currency = source_currencies.id')
      .where('source_currencies.code = ?', code)
  end

  def self.target(code)
    joins('left join currencies target_currencies on historical_rates.target_currency = target_currencies.id')
  ...
  def self.from_client_response(response)
    ...
    return false if HistoricalRate.on(response[:date]).for(response[:base]).last.present?
